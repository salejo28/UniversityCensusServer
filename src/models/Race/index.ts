...
export default class RaceModel implements RaceModelUI {
  public async getAll() {
    ...
    const result = await Query(
      connection,
      "SELECT race.id, race.name, race.dangerous, race.createdAt, specie.name AS specie FROM race INNER JOIN specie ON race.specie = specie.id"
    );
    connection.end();
    return result;
  }
  ...
  public async getById(id: number | string) {
    ...
    const result = await Query(
      connection,
      "SELECT race.id, race.name, race.dangerous, race.createdAt, specie.name AS specie FROM race INNER JOIN specie ON race.specie = specie.id WHERE race.id = ?",
      id
    );
    connection.end();
    return result[0];
